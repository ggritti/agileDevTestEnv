...
public class MyLambdaExample {
    ...
    public static void createStringFilteredList() {
        List<String> strList = Arrays.asList("abc", " ", "bctestd", " ", "efg", "jkefg");
        List<String> filtered = strList.stream().filter((x) -> (x.length() > 2 && x.contains("test")) || x.contentEquals("efg"))
                .collect(Collectors.toList());
        System.out.printf("Original List : %s%nFiltered list : %s %n", strList, filtered);
    }
